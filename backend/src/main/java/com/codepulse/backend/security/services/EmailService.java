...
public class EmailService {
     ...
     */
    public void sendFeedbackEmail(String name, String username, String email, String category, int rating, String message) {
        String cleanKey = brevoApiKey.replaceAll("\\s+", "").trim();

        if (cleanKey.isEmpty()) {
            logger.error("BREVO_API_KEY is not configured! Cannot send feedback email.");
            return;
        }

        logger.info("Sending feedback email from username '{}' ({})", username, email);
        ...
        String htmlContent = "<!DOCTYPE html><html><head><meta charset='UTF-8'></head>"
            ...
            + "<table width='100%' cellpadding='0' cellspacing='0' style='max-width:520px;background-color:#ffffff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);'>"
            + "<tr><td style='background:linear-gradient(135deg,#7c3aed,#3b82f6);padding:32px 40px;text-align:center;'>"
            + "<h1 style='margin:0;color:#ffffff;font-size:24px;font-weight:700;letter-spacing:-0.5px;'>New Feedback Received</h1>"
            + "<p style='margin:8px 0 0;color:rgba(255,255,255,0.85);font-size:13px;'>CodePulse User Feedback Submission</p></td></tr>"
            + "<tr><td style='padding:36px 40px 24px;'>"
            + "<table width='100%' cellpadding='0' cellspacing='0' style='margin-bottom:20px; font-size:14px;'>"
            + "<tr><td style='padding: 6px 0; color: #64748b; font-weight: 600;' width='120'>Full Name:</td><td style='padding: 6px 0; color: #1a1a2e;'>" + escapeHtml(name != null && !name.isBlank() ? name : "Not Provided") + "</td></tr>"
            + "<tr><td style='padding: 6px 0; color: #64748b; font-weight: 600;'>Username:</td><td style='padding: 6px 0; color: #1a1a2e;'>" + escapeHtml(username) + "</td></tr>"
            + "<tr><td style='padding: 6px 0; color: #64748b; font-weight: 600;'>Email:</td><td style='padding: 6px 0; color: #1a1a2e;'>" + escapeHtml(email) + "</td></tr>"
            + "<tr><td style='padding: 6px 0; color: #64748b; font-weight: 600;'>Category:</td><td style='padding: 6px 0; color: #1a1a2e; text-transform: capitalize;'>" + escapeHtml(category) + "</td></tr>"
            + "<tr><td style='padding: 6px 0; color: #64748b; font-weight: 600;'>Rating:</td><td style='padding: 6px 0; color: #eab308; font-weight: 700;'>" + rating + " / 5 ★</td></tr>"
            + "</table>"
            + "<div style='background-color:#f8fafc;border-radius:8px;padding:16px;border:1px solid #e2e8f0;margin-top:10px;'>"
            + "<p style='margin:0 0 6px;color:#64748b;font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;'>Feedback Message:</p>"
            + "<p style='margin:0;color:#334155;font-size:13.5px;line-height:1.6;white-space:pre-wrap;'>" + escapeHtml(message) + "</p>"
            + "</div></td></tr>"
            + "<tr><td style='padding:20px 40px 28px;border-top:1px solid #f1f5f9;text-align:center;'>"
            + "<p style='margin:0;color:#94a3b8;font-size:11px;'>&copy; 2026 CodePulse. All rights reserved.</p>"
            + "</td></tr></table></td></tr></table></body></html>";
        ...
        String jsonBody = String.format("""
            {
              "sender": {"name": "%s", "email": "%s"},
              "to": [{"email": "abdulkarimazad73@gmail.com", "name": "Abdul Karim Azad"}],
              "subject": "CodePulse Feedback [%s] — %s",
              "htmlContent": %s,
              "headers": {
                "X-Entity-Ref-ID": "%s",
                "X-Mailer": "CodePulse/%s"
            ...
            escapeJson(fromName),
            escapeJson(fromEmail),
            escapeJson(category.toUpperCase()),
            timestamp,
            toJsonString(htmlContent),
            uniqueId,
            uniqueId
        ...
        try {
            ...
            if (status >= 200 && status < 300) {
                ...
                logger.info("Feedback email sent successfully to abdulkarimazad73@gmail.com — status: {}, response: {}", status, responseBody);
            } else {
                try (BufferedReader br = new BufferedReader(new InputStreamReader(conn.getErrorStream(), StandardCharsets.UTF_8))) {
                    StringBuilder sb = new StringBuilder();
                    String line;
                ...
                logger.error("Brevo API error for feedback email — status: {}, response: {}", status, responseBody);
            }
            conn.disconnect();
        } catch (IOException e) {
            logger.error("Failed to send feedback email to admin: {} — {}", e.getClass().getSimpleName(), e.getMessage(), e);
        }
    }
}
